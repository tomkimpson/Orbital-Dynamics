...
public transform_to_comoving_frame, PostKeplerianDelays, transform_BL_to_XY
...
vXY(1) = r/m * sin(theta)*cos(phi) * vBL(1) + m*cos(theta)*cos(phi)*vBL(2) - m*sin(theta)*sin(phi)*vBL(3)




vXY(2) = r/m * sin(theta)*sin(phi) * vBL(1) + m*cos(theta)*sin(phi)*vBL(2) + m*sin(theta)*cos(phi)*vBL(3)


vXY(3) = cos(theta)*vBL(1) -r*sin(theta)*vBL(2)
